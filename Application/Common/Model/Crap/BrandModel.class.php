...
class BrandModel extends BaseModel
{
     ...
     * @var string 
     */
    protected $trueTableName = 'crap_brand';
     ...
     * @return boolean
     */
    public function getByMobile($mobile, $fieldList=[])
    {
     ...
     * @return    array
     */
    public function getDataByWhere($where=[],$page=1,$pagenum=30,$order=" create_time desc ")
    {
        $list['data']  = $this->where($where)->order($order)->page($page.','.$pagenum)->select();
        $list['count'] = $this->where($where)->count();// 查询满足要求的总记录数
        $list['show']  = $this->formatPage($list['count'],$pagenum);
    ...
    {
        $all = 'id';
        $data = $this->where($where)->Field($all)->getAll();
        return $data ;
    }    
}
